...
def combine_heatmaps(krt_density_raw, nonkrt_density_raw, output_dir, bandwidth, gini_krt, gini_nonkrt):
...
def estimate_kde_for_tissue(coords, tissue_type, output_dir,
...
def estimate_kde(krt_coords, nonkrt_coords, output_dir, reference_image_path, bandwidth, tile_level, grid_resolution,
                 map_size):
    ...
    if len(nonkrt_coords) >= 2:
        heatmap_nonkrt, gini_nonkrt, nonkrt_density, nonkrt_density_raw = estimate_kde_for_tissue(
            nonkrt_coords, "nonkrt", output_dir,
            reference_image_path, bandwidth, tile_level, grid_resolution, map_size
        )
    ...
    if krt_density is not None and nonkrt_density is not None:
        combined_tiff = combine_heatmaps(krt_density_raw, nonkrt_density_raw, output_dir,
                                         bandwidth, results['krt']['gini'], results['nonkrt']['gini'])
...
for sample in samples:
    sample['folder'] = windows_to_wsl_path(sample['folder'])
        ...
        try:
            ...
            with open(results_file, 'w', newline='') as f:
                ...
                for bw in [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1]:
                    print(f"\n{'=' * 60}")
                    print(f"Processing bandwidth: {bw}")
                    print(f"{'=' * 60}")

                    results = estimate_kde(krt_coords, nonkrt_coords,
                                           output_dir, ref_tiff,
                                           bandwidth=bw, tile_level=2, grid_resolution=200, map_size=1000
                                           )
