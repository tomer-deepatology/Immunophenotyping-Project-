...
base_img = Image.open(img_path).convert('RGBA').resize((800, 800))
x, y = df['x_local'].values, df['y_local'].values
xy = np.vstack([x, y])

w, h = base_img.size
xx, yy = np.meshgrid(np.linspace(0, df['x_local'].max(), 200), np.linspace(0, df['y_local'].max(), 200))
...
bw_slider = tk.Scale(root, from_=0.01, to=1.0, resolution=0.01, orient=tk.HORIZONTAL,
                     length=600, width=20, command=lambda v: update_display())
bw_slider.set(0.1)
